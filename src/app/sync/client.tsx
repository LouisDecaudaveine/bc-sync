...
export function SyncClient() {
  ...
  // When sync finishes, invalidate releases so they refetch with new data.
  const prevRunning = useRef<boolean | undefined>(undefined);
  useEffect(() => {
    if (prevRunning.current === true && status && !status.running) {
  ...
  return (
    <div className="max-w-2xl mx-auto py-12">
      ...
      <div className="rounded border border-neutral-800 p-6 bg-neutral-900/50">
        ...
        {syncMutation.error && (
          <p className="mt-4 text-sm text-red-400">{syncMutation.error.message}</p>
        )}
        ...
        <div className="mt-6 flex gap-3">
          {!running && (
            ...
            >
              {syncMutation.isPending ? "Starting…" : isDone || isError ? "Sync again" : "Start sync"}
            </button>
          )}
          {isDone && (
            <Link
